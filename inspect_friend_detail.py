...
def ensure_friends(driver):
    ...
    if driver.find_elements(By.CSS_SELECTOR, "input[type='password']"):
        dump_state(driver, "inspect_before_login")
...
def main() -> None:
    ...
    try:
        ...
        time.sleep(2)
        dump_state(driver, f"inspect_search_{query}")

        rows = driver.find_elements(By.CSS_SELECTOR, ".friendlistItem-module__button_friendlist_item__xoWur")
        profiles = driver.find_elements(By.CSS_SELECTOR, ".profileImage-module__button_profile__GqKue")
        print(f"row_count={len(rows)} profile_count={len(profiles)}")
        if index >= len(rows):
            raise IndexError(f"row index {index} out of range")
        driver.execute_script("arguments[0].click();", rows[index])
        time.sleep(2)
        dump_state(driver, f"inspect_profile_{query}_{index}")
        name_buttons = driver.find_elements(By.CSS_SELECTOR, ".chatroomHeader-module__button_name__US7lb")
        print(f"header_name_button_count={len(name_buttons)}")
        if name_buttons:
            driver.execute_script("arguments[0].click();", name_buttons[0])
            time.sleep(2)
            dump_state(driver, f"inspect_header_profile_{query}_{index}")

        print("html_excerpt_begin")
        print(driver.execute_script("return document.body.innerHTML.slice(0, 8000);"))
